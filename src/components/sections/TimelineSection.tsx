...
export const TimelineSection = () => {
  ...
  return (
    <section id="experience" className="section-padding relative">
      <div className="max-w-3xl mx-auto"> {/* Lebar ditambah sedikit agar muat gambar */}
        <div className="text-center mb-12" data-aos="fade-up">
          <span className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Journey</span>
          <h2 className="text-3xl md:text-4xl font-bold mt-3 text-foreground tracking-tight">EXPERIENCE</h2>
        </div>
        ...
        <div data-aos="fade-up" data-aos-delay="150">
          <div className="rounded-xl border border-border bg-card/50 overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border/50 bg-card/80">
              ...
              <span className="text-[10px] text-muted-foreground tracking-wider flex-1 text-center font-mono">career.log</span>
              <div className="w-12" />
            </div>

            <div className="p-2 md:p-4 space-y-1 font-mono">
              {experiences.map((exp, index) => (
                <div key={exp.id || `${exp.year}-${index}`} data-aos="fade-left" data-aos-delay={200 + index * 100}>
                  <div
                    className={cn(
                      "group rounded-lg cursor-pointer transition-all duration-300",
                      index === activeIndex ? "bg-accent/30" : "hover:bg-accent/10",
                  ...
                  >
                    ...
                    <div className="px-4 py-3 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground flex items-center gap-2">
                          <span className="text-foreground/80">{exp.title}</span>
                          <span className="opacity-30">@</span>
                          <span className="text-primary/70">{exp.company}</span>
                        </div>
                      </div>
                    ...
                    {/* Expanded Content */}
                    <div className={cn("grid transition-all duration-500 ease-in-out", expandedIndex === index ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0")}>
                      <div className="overflow-hidden">
                        <div className="px-4 pb-6 pl-11 flex flex-col md:flex-row gap-6">
                          ...
                          <div className="flex-1 space-y-4">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {exp.skills.map((skill, i) => (
                                <span key={`skill-${index}-${i}`} className="px-2 py-0.5 text-[9px] bg-primary/10 text-primary/80 border border-primary/20 rounded">
                                  {skill}
                                </span>
                              ))}
                            </div>
                          ...
                          {/* Pastikan di schema database ada field 'image' */}
                              ...
                              {exp.image && (
                                <Image
                                  ...
                                  className={cn(
                                    ...
                                    imageLoading[exp.id] ? "opacity-0" : "opacity-100"
                                  )}
